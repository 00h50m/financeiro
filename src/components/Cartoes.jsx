...
export default function Cartoes({ store }) {
  const { cartoes, addCartao, delCartao } = store
  ...
  async function salvar() {
    if (!form.nome) return
    setSaving(true)
    await addCartao({ nome: form.nome, titular: form.titular, fechamento: Number(form.fechamento) || 1, vencimento: Number(form.vencimento) || 10, ativo: true })
    setSaving(false)
    setModal(false)
    ...
    <div className="page">
      {modal && (
        <div className="overlay" onClick={(e) => { if (e.target.className === 'overlay') setModal(false) }}>
          <div className="modal">
            <div className="modal-title">Novo cartão</div>
      ...
      <div style={{ marginBottom: 16 }}>
        <button className="btn btn-primary" onClick={() => setModal(true)}>+ Novo cartão</button>
      </div>
